defmodule BandwidthHero.ContractorsTest do
  ...
  describe "contractors" do
    ...
    test "create_contractor/1 with valid data creates a contractor" do
      valid_attrs = %{availability: :full, bandwidth: 42, contract_type: :corp_to_corp, international_travel: :yes, laptop: :use_my_own, name: "some name", title: "some title", travel: :"100%"}

      assert {:ok, %Contractor{} = contractor} = Contractors.create_contractor(valid_attrs)
      assert contractor.availability == :full
      assert contractor.bandwidth == 42
    ...
    test "update_contractor/2 with valid data updates the contractor" do
      ...
      assert contractor.availability == :partial
      assert contractor.bandwidth == 43
